...
def main():
    ...
    The model assumes:
    - Enrollment follows a structured cohort-based system.
    - Retention rates impact year-to-year student counts.
    - There is uncertainty in new enrollments and retention.
    - The user (you) wants to control the total enrollment so that it follows a straight line with a specified slope.
    ...
    By adjusting these values, you can explore different enrollment trend scenarios.
                
    You can see that the total enrollment will follow a line with an R-squared very close to 1.  You can also see that the first-year cohort sizes can vary significantly (appearing noisy) when in fact they are being carefully calculated!
    """)
